...
func run() error {
	...
	middleAngle, err := strconv.Atoi(args[4])
	if err != nil {
		return err
	}
	outDir := args[5]
	return OverlapAdd(subject, soundName, moveWidth, moveVelocity, middleAngle, outDir)
}

func OverlapAdd(subject, soundName string, moveWidth, moveVelocity, middleAngle int, outDir string) error {
	...
	for _, direction := range []string{"c", "cc"} {
		for _, LR := range []string{"L", "R"} {
			...
			// DDBへ出力
			outName := fmt.Sprintf("%s/move_judge_w%03d_mt%03d_%s_%d_%s.DDB", outDir, moveWidth, moveVelocity, direction, middleAngle, LR)
...
func calcAngles(moveWidth, middleAngle int, clockwise bool) []int {
	var startAngle int
	if clockwise {
		startAngle = middleAngle - moveWidth/2
	} else {
		startAngle = middleAngle + moveWidth/2 - 1
	}
	if startAngle < 0 {
		startAngle += 3600
	}
	startAngle = startAngle % 3600
